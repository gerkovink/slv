...
output: 
  html_document:
    ...
    theme: paper
    highlight: pygments
    pandoc_args: --output=02_Data_Visualisation_answers.html
---

# Introduction

In this practical, we will learn how to visualise data after we have cleaned up our datasets using the `dplyr` verbs from the previous practical: `filter()`, `arrange()`, `mutate()`, `select()`, and `summarise()`. For the visualisations, we will be using a package that implements the grammar of graphics: `ggplot2`. Please review the lecture slide for week 2 beforehand.

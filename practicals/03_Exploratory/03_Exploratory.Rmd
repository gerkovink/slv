...
__11. Load the data `elastic1` and `elastic2` and bind the data frames together using the function `bind_rows()` and add a grouping variable indicating which data set a row stems from.__
...
__12. Create a scatterplot mapping `stretch` on the x-axis and `distance` on the y-axis, and map the just created grouping variable as the color aesthetic.__
...
__13. Recreate the previous plot, but now assess whether the results of the two data sets appear consistent by adding a linear regression line using `geom_smooth(type = "lm)`.__
...
- hand in the zip by PR from your fork [here](https://github.com/gerkovink/INFOMDA1-2021). Do so before Lecture 4. That way we can iron out issues during the next Q&A in Week 3. 
